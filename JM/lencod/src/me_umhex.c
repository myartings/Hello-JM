...
//My change
static const MotionVector TriRect[9] = {{0, 0}, {-4, 0}, {-4, 4}, {0, 4}, {4, 4}, {4, 0}, {4, -4}, {0, -4}, 
	{-4,-4}};
static const MotionVector DiamondOctagon[12] = {{-4, 0}, {4, 0}, {0, -4},{0, 4}, {-8, 4}, {-4, 8}, {4, 8}, {8, 4}, {8, -4}, {4, -8}, {-4, -8}, {-8, -4}};
//end change
                                   ...
                                   )
{
  ...
  //My change variable
  // center-biased flags of neighboring blocks
  short  L_CENTERBIASED , U_CENTERBIASED, RU_CENTERBIASED, LU_CENTERBIASED; 
  
  if(currMB->mbAvailA)
	  L_CENTERBIASED = p_Vid->mb_data[currMB->mbAddrA].isCenterBiased;
  else 
	  L_CENTERBIASED = 0;
  if(currMB->mbAvailB)
	  U_CENTERBIASED = p_Vid->mb_data[currMB->mbAddrB].isCenterBiased;
  else 
	  U_CENTERBIASED = 0;
  if(currMB->mbAvailC)
	  RU_CENTERBIASED = p_Vid->mb_data[currMB->mbAddrC].isCenterBiased;
  else 
	  RU_CENTERBIASED = 0;
  if(currMB->mbAvailD)
	  LU_CENTERBIASED = p_Vid->mb_data[currMB->mbAddrD].isCenterBiased;
  else 
	  LU_CENTERBIASED = 0;

  short  centerBiased = L_CENTERBIASED + U_CENTERBIASED + RU_CENTERBIASED + LU_CENTERBIASED;
...
sec_step: //modified Unsymmetrical-cross search
  iMinNow = best;
  
  int cross_step = 4;

  for(i = 4; i < search_range << 2; i+=cross_step)
  {
    search_step = i;
    cand.mv_x = (short) (iMinNow.mv_x + search_step);
    cand.mv_y = iMinNow.mv_y ;
  ...
  for(i = 4; i < (search_range << 1);i+=cross_step)
  {
    search_step = i;
    cand.mv_x = iMinNow.mv_x ;
    cand.mv_y = (short) (iMinNow.mv_y + search_step);
...
// search M word cross  
  ...
  int search_step_x = 0;
  int search_step_y = 0;
  for(i =4, j = 4; i < (search_range << 1); i+=cross_step_x, j+=cross_step_y)
  {
	  search_step_x = i;
	  search_step_y = j;
	  cand.mv_x = (short) (iMinNow.mv_x + search_step_x);
	  cand.mv_y = (short) (iMinNow.mv_y + search_step_y);
	  SEARCH_ONE_PIXEL
	  cand.mv_x = (short) (iMinNow.mv_x + search_step_x);
	  cand.mv_y = (short) (iMinNow.mv_y - search_step_y);
	  SEARCH_ONE_PIXEL
  ...
  //third_step:    // Uneven Multi-Hexagon-grid Search
  //My change 
  //sub step 1: 5x5 squre search , 3x3 squre search or octagon search 
  //depending on the center-biased flags of the neighboring blocks
  if(centerBiased == 4 || (centerBiased == 3 && LU_CENTERBIASED == 0))
  for(pos=1;pos<25;pos++)
  {
    cand.mv_x = iMinNow.mv_x + p_Vid->spiral_qpel_search[pos].mv_x;
    cand.mv_y = iMinNow.mv_y + p_Vid->spiral_qpel_search[pos].mv_y;
  ...
  else if(centerBiased == 3 || centerBiased == 2)
	  for (pos =1;pos< 12; pos++)
	  {
		 cand.mv_x = iMinNow.mv_x + DiamondOctagon[pos].mv_x;
		 cand.mv_y = iMinNow.mv_y + DiamondOctagon[pos].mv_y;
		SEARCH_ONE_PIXEL
	  }
  else 
	for(pos =1; pos<9; pos++)
	{
		cand.mv_x = iMinNow.mv_x + TriRect[pos].mv_x;
		cand.mv_y = iMinNow.mv_y + TriRect[pos].mv_y;
		SEARCH_ONE_PIXEL
	}

  if ((min_mcost - p_UMHex->pred_SAD)<p_UMHex->pred_SAD * betaFourth_2                                                               || (min_mcost - p_UMHex->pred_SAD) < p_UMHex->pred_SAD * betaFourth_1)

	  currMB->isCenterBiased = 1;
  else
	  currMB->isCenterBiased = 0;
  //early termination alogrithm, refer to JVT-G016
  EARLY_TERMINATION
